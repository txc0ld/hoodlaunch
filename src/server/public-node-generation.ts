...
import { digest, publicSessionIdentity, fail, jsonBody, readToken, writeGuard } from './public-security';
...
export async function handleNodeGeneration(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = body.action === 'status' ? ['action', 'expectedSessionIdentity'] : ['action', 'requestId', 'count', 'expectedSessionIdentity'];
  ...
  const sessionIdentity = publicSessionIdentity(token);
  if (body.expectedSessionIdentity !== sessionIdentity) fail(409, 'SESSION_CHANGED', 'Your account session changed. Refresh before creating a wallet.');
  ...
  if (body.action === 'status') {
    ...
    res.json({ sessionIdentity, enabled:true, maxCount:pro?50:1, available:pro || data.available, nextEligibleAt:pro?null:data.nextEligibleAt }); return;
  }
