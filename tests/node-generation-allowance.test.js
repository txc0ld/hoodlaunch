...
test('allowance migration provides service-only atomic cooldown, idempotence and account isolation',async()=>{
 const db=new PGlite();try{
  ...
  const reserve=async(u,s,id,count=1,pro=false)=>(await db.query('select hood_node_generation_reserve($1,$2,$3,$4,$5) as result',[u,s,id,count,pro])).rows[0].result;
  ...
  await assert.rejects(reserve(user,token,other,2,false),/invalid generation/);
  ...
  assert.equal((await reserve(user,token,other,50,true)).count,50);
...
test('generation route enforces closed flag, identity, free count and unknown results',async()=>{
 ...
 const handler=security.safeHandler(load('src/server/public-node-generation.ts',{'./public-security':security,'./public-services':{account:async()=>{calls++;return user;},takeQuota:async()=>{},getProStatus:async()=>({pro}),database:()=>({rpc:async(name,args)=>{assert.equal(args.p_user,user);assert.equal(args.p_session,security.digest(token));if(name.endsWith('reserve'))assert.equal(args.p_pro,pro);return reply;}})}}).handleNodeGeneration);
 try{
