...
function req(body){const r=new PassThrough();r.method='POST';r.rawHeaders=['Origin','https://test.example'];r.headers={origin:'https://test.example','content-type':'application/json',cookie:`${security.cookieName()}=${token}`};process.nextTick(()=>r.end(JSON.stringify({expectedSessionIdentity:security.publicSessionIdentity(token),...body})));return r;}
...
test('generation route enforces closed flag, identity, free count and unknown results',async()=>{
 ...
 let quotaCalls=0,calls=0,pro=false,reply={data:{reserved:true,requestId,count:1,reservedAt:'2026-09-11T00:00:00Z',nextEligibleAt:'2026-09-12T00:00:00Z'},error:null};
 const handler=security.safeHandler(load('src/server/public-node-generation.ts',{'./public-security':security,'./public-services':{account:async()=>{calls++;return user;},takeQuota:async()=>{quotaCalls++;},getProStatus:async()=>({pro,subscription:pro}),database:()=>({rpc:async(name,args)=>{assert.equal(args.p_user,user);assert.equal(args.p_session,security.digest(token));if(name==='hood_holder_context')return {data:{address:null,proof_id:null},error:null};if(name.endsWith('reserve')){assert.equal(args.p_pro,pro);if(!pro&&args.p_count===2)return {data:{reserved:false,code:'PRO_REQUIRED'},error:null};}return reply;}})}}).handleNodeGeneration);
 try{
  ...
  for (const expectedSessionIdentity of [undefined, 'c'.repeat(64), security.digest(token), token]) { const r=res(); await handler(req({action:'reserve',requestId,count:1,expectedSessionIdentity}),r); assert.equal(r.statusCode,409); assert.equal(quotaCalls,0); }
  let r=res();await handler(req({action:'reserve',requestId,count:2}),r);assert.equal(r.statusCode,403);
  r=res();await handler(req({action:'reserve',requestId,count:1}),r);assert.equal(r.statusCode,200);assert.equal(r.body.count,1);assert.equal(r.body.sessionIdentity,security.publicSessionIdentity(token));
