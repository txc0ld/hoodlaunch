...
const componentText = fs.readFileSync(path.join(ROOT, 'src/components/NodeTrading.tsx'), 'utf8')
  .replace('const STATUS_REFRESH_DEADLINE_MS = 20_000;', 'const STATUS_REFRESH_DEADLINE_MS = 600;')
  .replace('const deadline = Date.now() + STATUS_REFRESH_DEADLINE_MS;', 'const deadline = Date.now() + (globalThis.__statusRefreshDeadlineMs || STATUS_REFRESH_DEADLINE_MS);');
...
const state={scenario,refreshCalls:0,refreshByNode:{},refreshByHash:{},refreshActiveByHash:{},refreshMaxByHash:{},activeRefreshes:0,maxRefreshes:0,snapshotCalls:0,snapshotFailures:0,nodeBalanceCallbacks:0,batchPrepares:[],batchExecutes:[],singleExecutes:0,batchProgress:0,holdBatch:false,hangReads:scenario==='coalescing',hungResolvers:[],heldStatusNode:'',heldStatusResolvers:[],heldStatusActive:0,confirmed:false,confirmedNodes:new Set(),confirmPrevious:false,slowNode:nodes[0],failNode:'',failNodeCount:0,operations:{}};
...
if(!['partial','partial-approval','partial-unknown','approval-error','batch-replaced','single-review-hash'].includes(scenario))state.operations[nodes[0].toLowerCase()]=operation(nodes[0],'sell','pending',PREVIOUS_TOKEN,50);
const modules={
 ...
 '../lib/node-trading':{
  ...
  refreshNodeTradeOperation:async node=>{const key=node.toLowerCase(),started=state.operations[key],hash=started?.txHash||'missing';state.refreshCalls++;state.refreshByNode[key]=(state.refreshByNode[key]||0)+1;state.refreshByHash[hash]=(state.refreshByHash[hash]||0)+1;state.refreshActiveByHash[hash]=(state.refreshActiveByHash[hash]||0)+1;state.refreshMaxByHash[hash]=Math.max(state.refreshMaxByHash[hash]||0,state.refreshActiveByHash[hash]);state.activeRefreshes++;state.maxRefreshes=Math.max(state.maxRefreshes,state.activeRefreshes);try{if(state.hangReads)await new Promise(resolve=>state.hungResolvers.push(resolve));await new Promise(resolve=>setTimeout(resolve,node===state.slowNode?250:15));if(state.heldStatusNode===key){state.heldStatusActive++;try{await new Promise(resolve=>state.heldStatusResolvers.push(resolve));}finally{state.heldStatusActive--;}}if(state.failNode===key&&state.failNodeCount>0){state.failNodeCount--;throw Error('Temporary status RPC failure');}const current=state.operations[key];if(!current)throw Error('No recorded trade exists for this node.');if((current.tokenAddress===PREVIOUS_TOKEN&&state.confirmPrevious)||(current.tokenAddress===TOKEN&&(state.confirmed||state.confirmedNodes.has(key))))state.operations[key]={...current,status:'confirmed',message:'Trade confirmed on Robinhood Chain. Refresh holdings for the latest balances.'};return state.operations[key].txHash===started.txHash?state.operations[key]:started;}finally{state.activeRefreshes--;state.refreshActiveByHash[hash]--;}}
 },
 '../lib/pons-trade':{
 ...
 '../lib/node-trade-batch':{
  prepareNodeTradeBatch:async(current,token,side,assertCurrent)=>{assertCurrent();state.batchPrepares.push(side);const approvalScenario=['partial-unknown','approval-error'].includes(state.scenario);const entries=nodes.map((node,index)=>{const existing=state.operations[node.toLowerCase()];if(existing&&['pending','unknown'].includes(existing.status))return {nodeIndex:index,nodeAddress:node,status:'blocked',error:'Check the recorded transaction before another action.'};const item=review(node,index,side);if(state.scenario==='approval-error'&&index===0)return {nodeIndex:index,nodeAddress:node,status:'deferred',review:item,error:'This sale is deferred until the approval-only batch is confirmed.'};return {nodeIndex:index,nodeAddress:node,status:'ready',review:approvalScenario?{...item,action:'approve-token'}:item};});const ready=entries.filter(entry=>entry.status==='ready');return {tokenAddress:token,side,mode:approvalScenario?'approvals':'trades',entries,expiresAt:ready.length?Date.now()+30000:0,maxGasCostWei:String(ready.length*100000000000000),maxTotalEthWei:String(ready.length*100000000000000)};},
  executeNodeTradeBatch:async(current,batch,assertCurrent,onProgress)=>{assertCurrent();state.batchExecutes.push(batch.side);if(state.scenario==='approval-error'){/* Deterministic test substitute for the wallet/RPC result rows; no transaction is signed. */const results=[{nodeIndex:0,nodeAddress:nodes[0],status:'not-submitted',error:'This sale is deferred until the approval-only batch is confirmed.'},{nodeIndex:1,nodeAddress:nodes[1],status:'error',error:'Balance, gas or nonce changed. Prepare a fresh trade.'},...nodes.slice(2).map((node,index)=>({nodeIndex:index+2,nodeAddress:node,status:'not-submitted',error:'Stopped after a node could not be submitted.'}))];for(const result of results){state.batchProgress++;onProgress(result);}return results;}if(state.scenario==='partial-unknown'){/* Deterministic unknown-receipt substitute; no transaction is signed. */const op=operation(nodes[0],batch.side,'unknown',TOKEN,10,'approve-token');state.operations[nodes[0].toLowerCase()]=op;const results=[{nodeIndex:0,nodeAddress:nodes[0],status:'unknown',operation:op},...nodes.slice(1).map((node,index)=>({nodeIndex:index+1,nodeAddress:node,status:'not-submitted',error:'Stopped after a node transaction had an unknown outcome.'}))];for(const result of results){state.batchProgress++;onProgress(result);}return results;}if(['partial','partial-approval','batch-replaced'].includes(state.scenario)){const approval=state.scenario==='partial-approval',op=operation(nodes[0],batch.side,'pending',TOKEN,10,approval?'approve-token':batch.side);state.operations[nodes[0].toLowerCase()]=op;const results=[{nodeIndex:0,nodeAddress:nodes[0],status:'submitted',operation:op},{nodeIndex:1,nodeAddress:nodes[1],status:'error',error:'Node gas balance is below the reviewed maximum cost.'},...nodes.slice(2).map((node,index)=>({nodeIndex:index+2,nodeAddress:node,status:'not-submitted',error:'Stopped after a node could not be submitted.'}))];for(const result of results){state.batchProgress++;onProgress(result);}return results;}const results=[];for(const entry of batch.entries){if(entry.status!=='ready')continue;const op=operation(entry.nodeAddress,batch.side);state.operations[entry.nodeAddress.toLowerCase()]=op;const result={nodeIndex:entry.nodeIndex,nodeAddress:entry.nodeAddress,status:'submitted',operation:op};results.push(result);state.batchProgress=results.length;onProgress(result);if(state.holdBatch&&results.length===1)await new Promise(resolve=>window.releaseHeldBatch=resolve);assertCurrent();}return results;}
 }
};
...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => test.state.heldStatusActive === 1 && Array.from(document.querySelectorAll('strong')).some(node => node.textContent === 'Trade confirmed on Robinhood Chain'));
    const progressWhileHeld = await page.locator('[role="status"]').filter({ hasText: /Checking transactions/ }).textContent();
    const activeWhileHeld = await page.evaluate(() => test.state.activeRefreshes);
    await page.evaluate(() => test.releaseHeldStatus());
    assert.ok(activeWhileHeld > 0, 'Fast confirmations publish while the targeted status response is held');
    assert.match(progressWhileHeld, /Checking transactions [1-3] of 4/, 'The shared four-node refresh cycle reports bounded progress while one targeted response is held');
    ...
    try {
      const approvalErrorPage=await context.newPage();approvalErrorPage.on('pageerror',error=>errors.push(error.message));
      await approvalErrorPage.goto('http://localhost:41924/?scenario=approval-error');
      await approvalErrorPage.getByRole('button',{name:'Sell Max All Nodes',exact:true}).click();
      const approvalBanner=approvalErrorPage.locator('[aria-labelledby="node-batch-review-title"] [role="alert"]');
      await approvalBanner.waitFor();
      const approvalBannerText=await approvalBanner.textContent();
      assert.match(approvalBannerText,/Node 2/i,'The banner identifies the approval node that failed');
      assert.match(approvalBannerText,/Balance, gas or nonce changed\. Prepare a fresh trade\./,'The real approval error takes precedence over the earlier deferred sale');
      assert.match(approvalBannerText,/1 sale was deliberately deferred/i,'The deferred sale is counted separately');
      assert.match(approvalBannerText,/1 approval failed and 3 actions were skipped/i,'The failed approval and skipped actions are distinguished');
      assert.equal(await approvalErrorPage.getByText(/Approvals confirmed/).count(),0,'An all-unsent batch never reports confirmed approvals');
      assert.deepEqual(await approvalErrorPage.evaluate(()=>test.state.batchExecutes),['sell'],'The summary does not retry or advance the approval-only batch');
      console.log(JSON.stringify({pass:true,check:'all-unsent approval summary prioritizes the failed node',bannerText:approvalBannerText}));
      await approvalErrorPage.close();
    } catch(error) { repairFailures.push('all-unsent approval error summary: '+error.message); }
    try {
      const unknownPage=await context.newPage();unknownPage.on('pageerror',error=>errors.push(error.message));
      await unknownPage.goto('http://localhost:41924/?scenario=partial-unknown');
      await unknownPage.getByRole('button',{name:'Sell Max All Nodes',exact:true}).click();
      const unknownBanner=unknownPage.locator('[aria-labelledby="node-batch-review-title"] [role="alert"]');
      await unknownBanner.waitFor();
      const unknownBannerText=await unknownBanner.textContent();
      assert.match(unknownBannerText,/Node 1/i,'The banner identifies the node with the unknown receipt');
      assert.match(unknownBannerText,/Do not resubmit/i,'The unknown receipt keeps its no-resubmission instruction');
      assert.doesNotMatch(unknownBannerText,/choose Sell|try again|retry/i,'An unknown receipt does not recommend another submission');
      assert.equal(await unknownPage.getByText(/Approvals confirmed/).count(),0,'An unknown approval is never reported as confirmed');
      assert.deepEqual(await unknownPage.evaluate(()=>test.state.batchExecutes),['sell'],'The unknown summary does not retry or advance the batch');
      console.log(JSON.stringify({pass:true,check:'unknown approval receipt keeps node identity and blocks retry advice',bannerText:unknownBannerText}));
      await unknownPage.close();
    } catch(error) { repairFailures.push('unknown approval summary: '+error.message); }
    try {
