...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => __resolve(0, 'ipfs://account-a'));
    await page.waitForTimeout(0);
    assert.equal(await uriInput.inputValue(), '');
    assert.equal(await page.evaluate(() => __test.uploads[0].signal.aborted), true);
    assert.ok((await page.evaluate(() => __test.revoked)).includes('blob:fixture-1'));
    ...
    await page.evaluate(() => __resolve(2, 'ipfs://newer'));
    await page.waitForFunction(() => document.querySelector('[aria-label="Token image URI"]').value === 'ipfs://newer');
    await page.evaluate(() => __resolve(1, 'ipfs://older'));
    await page.waitForTimeout(0);
    assert.equal(await uriInput.inputValue(), 'ipfs://newer');
    assert.equal(await page.evaluate(() => __test.uploads[1].signal.aborted), true);
    ...
    await page.evaluate(() => __resolve(3, 'ipfs://removed'));
    await page.waitForTimeout(0);
    assert.equal(await uriInput.inputValue(), '');
    ...
    await page.evaluate(() => __resolve(4, 'ipfs://pending-before-invalid'));
    await page.waitForTimeout(0);
    assert.equal(await uriInput.inputValue(), '');
    assert.equal(await page.evaluate(() => __test.uploads[4].signal.aborted), true);
    assert.deepEqual(errors, []);
    console.log(JSON.stringify({ check: 'account, replacement, removal, and invalid-newer selection fence stale upload completion', pass: true, uploads: 5 }));
    await context.close();
  } finally {
    await browser.close();
  }
