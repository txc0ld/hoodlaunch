...
import { HOODRICH_MINIMUM_FORMATTED, PRO_PRICE_LABEL } from '../src/lib/pro-access';
...
const tiers = [
 ...
 ['Generated wallets with verified encrypted backups', '1 per account every 24 hours', 'Up to 50 per attempt'],
...
export default function Pro() {
 return <main className={styles.page}>
  ...
  <header><p className={styles.eyebrow}>FREE OR {PRO_PRICE_LABEL}</p><h1>Launch free. Scale when you need it.</h1><p>Free includes connected-wallet launching and core planning. Free accounts also get one wallet generation attempt every 24 hours when wallet creation is available. Pro adds bulk wallets, comparisons and managed uploads.</p></header>
  ...
  <section id="compare"><h2>Compare Free and Pro</h2><div className={styles.table}><table><thead><tr><th>Feature</th><th>Free</th><th>Pro</th></tr></thead><tbody>{tiers.map(row=><tr key={row[0]}>{row.map((cell,index)=>index===0?<th key={cell} scope="row">{cell}</th>:<td key={cell+index}>{cell}</td>)}</tr>)}</tbody></table></div><p>Generation and live tools are available only after their release checks. Restoring an existing backup does not use the daily allowance or require Pro.</p><p>Token Lab is a local, generic planning tool. It does not feed values into PONS, deploy a token, allocate supply, change fees or predict returns.</p><p>A custom launch pair must be approved by PONS and pass factory, decimals and economics checks. It is fixed at creation, forces the initial developer buy to zero, and still needs native ETH for launch fees and gas. Later custom-pair trading uses PONS links; generated-node trading remains for ETH pairs.</p></section>
  <section id="start"><h2>Get started in four steps</h2><ol><li>Use the free launch form and Token Lab with a small test plan.</li><li>Sign in with your wallet, or use your existing email account, when account services are available.</li><li>Subscribe for {PRO_PRICE_LABEL} or verify one wallet holding at least {HOODRICH_MINIMUM_FORMATTED} HOODRICH ($RICH).</li><li>Return to the workspace. Pro appears only after current access is verified.</li></ol></section>
  <section id="holder"><h2>Unlock Pro with HOODRICH</h2><ol><li>Sign in to your account.</li><li>Connect the wallet holding $RICH on Robinhood Chain.</li><li>Select <strong>Verify holding wallet</strong> and sign the ownership message.</li></ol><p>The signature moves no tokens, requests no spending approval and costs no gas. Exactly {HOODRICH_MINIMUM_FORMATTED} $RICH qualifies. Holdings in different wallets are not combined.</p><details><summary>Holder access rules</summary><p>The balance is checked for managed services and about every minute in the workspace. Falling below the threshold ends holder access on the next check unless an active paid subscription qualifies separately. Verify again after a new sign-in. One standard wallet can link to one account; smart contract and delegated wallets are not supported for this proof yet.</p><p>Becoming eligible does not cancel an existing subscription or create a refund. Manage billing separately.</p></details></section>
  <section id="billing"><h2>Subscription and operating costs</h2><p>HOODLABS Pro is {PRO_PRICE_LABEL}. It supports the hosted workflow and managed services without adding an application charge to each on-chain trade.</p><ul><li>Stripe Checkout shows the recurring price and terms before purchase.</li><li>If billing is unavailable, subscribing stays disabled.</li><li>Network gas, PONS fees, creator tax, exchange withdrawals and Relay costs are separate.</li><li>Cancellation takes effect at the end of the paid period after Stripe confirms it.</li></ul><p>Use <strong>Manage subscription</strong> to review billing, renewal or cancellation. Pro stays unavailable while the service itself is disabled.</p></section>
