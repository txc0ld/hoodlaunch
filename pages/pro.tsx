...
const tiers = [
 ...
 ['Supported PONS launch plan', 'Included', 'Included'],
 ...
 ['Managed raster image uploads', '5 attempts per user per day', '50 attempts per user per day'],
...
export default function Pro() {
 return <main className={styles.page}>
  ...
  <header><p className={styles.eyebrow}>FREE OR {PRO_PRICE_LABEL}</p><h1>Create free. Scale when you need it.</h1><p>Every signed-in account can upload images and create one wallet every 24 hours when available. Pro raises those limits to 50 uploads per day and up to 50 wallets per attempt.</p></header>
  ...
  <section id="compare"><h2>Compare Free and Pro</h2><div className={styles.table}><table><thead><tr><th>Feature</th><th>Free</th><th>Pro</th></tr></thead><tbody>{tiers.map(row=><tr key={row[0]}>{row.map((cell,index)=>index===0?<th key={cell} scope="row">{cell}</th>:<td key={cell+index}>{cell}</td>)}</tr>)}</tbody></table></div><p>Generation and live tools are available only after their release checks. Restoring an existing backup does not use the daily allowance or require Pro.</p><p>A custom launch pair must be approved by PONS and pass factory, decimals and economics checks. It is fixed at creation, forces the initial developer buy to zero, and still needs native ETH for launch fees and gas. Later custom-pair trading uses PONS links; generated-node trading remains for ETH pairs.</p></section>
  <section id="start"><h2>Get started</h2><ol><li>Sign in with your wallet, or use your existing email account.</li><li>Create a token with the supported PONS settings.</li><li>Generate or restore wallets, then verify the encrypted backup before trading.</li></ol><p>Subscribe for {PRO_PRICE_LABEL} or prove ownership of a wallet holding at least {HOODRICH_MINIMUM_FORMATTED} HOODRICH ($RICH) to unlock Pro.</p></section>
