...
export default function Guide() {
 return <main className={styles.page}>
  ...
  <section id="trade"><h2>Trade an ETH-paired token</h2><ol><li>Enter and verify the full PONS token address, or use the address from a verified launch receipt.</li><li>Use one node’s 5%, 10%, 25%, 50%, Max, or custom action, or the overall Max All controls.</li><li>Each node submits separately. Exact approvals may require a fresh Sell action after confirmation.</li><li>Refresh pending or unknown transaction status before another action.</li></ol><p>Buy percentages use spendable ETH after gas. Sell percentages use token holdings. The 2% slippage cap does not remove price impact, fees, gas, or partial-fill risk.</p></section>

  <section id="recovery"><h2>Stop and recover when status is unclear</h2><ul><li><strong>Pending or unknown:</strong> preserve the transaction hash and refresh its recorded status. Do not resubmit.</li><li><strong>Expired quote:</strong> request a fresh quote only when nothing was submitted.</li><li><strong>Wallets locked:</strong> restore the verified encrypted backup. Submitted transactions continue onchain.</li><li><strong>Image rejected:</strong> use a still PNG, JPEG, or WebP under 4MB, or an existing public image URI.</li><li><strong>Pro unavailable:</strong> refresh account access and repeat holder proof after a new sign-in.</li></ul><p><a href="/security">Read the full security and recovery limits before using significant funds.</a></p></section>
 </main>;
}
