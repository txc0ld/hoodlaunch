...
function markers(p, row) { return ['intent-' + hash(p.id + ':' + row.index), 'destination-' + hash(row.address)]; }
...
async function execute(state, exchange, id, confirmation, {env = process.env, now = Date.now} = {}) {
  ...
  function fresh() { const time = now(); if (time < review.createdAt || time >= review.expiresAt) fail('Review expired. Prepare and inspect a fresh review.'); }
  ...
  try {
    const result = await exchange.withdraw(quote);
