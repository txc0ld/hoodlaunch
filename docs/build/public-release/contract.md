...
- Authentication is verified server-side. Account IDs never come from trusted client body/header claims. Supabase managed email OTP with HttpOnly Secure SameSite cookies; CSRF exact configured origin and method/content limits. No custom password storage.
