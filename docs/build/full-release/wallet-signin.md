...
- Fixed canonical-origin SIWE message, chain4663, checksummed address, nonce, request ID and five-minute expiration. Recover ECDSA signer before one native verification call. Verify native provider/address/chain/network/domain and user/session/identity UUID consistency. Native claims must be the non-array object at identity_data.custom_claims, with network exactly the string "4663", as observed from the hosted provider. Flat claims, numeric network IDs and user metadata are not accepted as authority. No EIP1271 or private-key handling.
