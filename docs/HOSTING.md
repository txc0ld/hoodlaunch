...
Current protected-release status: the dedicated Supabase project has reviewed migrations001/002 and Vercel Production has sensitive Supabase, wallet-grant and Stripe-key settings. The Stripe key's account and mode have not yet been independently verified, the webhook secret is not configured, and migration003 is intentionally unapplied. Supabase's free default email provider rejected the code-template change without custom SMTP or an upgrade, so public email-code sign-in remains blocked pending that service decision.
