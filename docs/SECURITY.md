...
| Duplicate checkout or event replay | Common retry condition | DB serializes checkout idempotency key; Stripe request key; entitlements never derived from webhook order or redirects. Checkout may require waiting after an expired/cancelled session. |
